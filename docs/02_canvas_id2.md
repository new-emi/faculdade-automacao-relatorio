...
| R9 | Repetição da mesma noticia na narracao (LLM reciclando itens para preencher blocos tematicos) e, depois da correcao, omissao de itens com listas longas | Roteiro e audio redundantes (depois, roteiro incompleto) — qualidade da entrega prejudicada | Diagnostico pelo roteiro gravado no banco + troca para formato linear, regra de mencao unica, proibicao de citar fonte/URL/data e repeat_penalty (1.2, repeat_last_n 1024) no Ollama; em seguida, reducao da edicao para ~10 noticias (5 por fonte, `MAX_POR_FONTE = 5`), porque com 20 o modelo de 3B passou a omitir itens, + regra de cobertura total no prompt; por fim, geracao em lotes de 5 noticias (`TAMANHO_LOTE = 5`) no `gerar_boletim`, que elevou a cobertura da edicao final para 10 de 10 itens sem repeticao (lote de 3 foi testado e descartado por piorar o resultado) | Resolvido (S13/S14/S15) |
