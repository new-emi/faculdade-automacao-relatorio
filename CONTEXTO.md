...
- Diagnostico e correcao da repeticao (tarefa 13): ouvindo o audio de audio/boletim_2026-09-17.wav (registro id=2), a narradora repete noticias. O roteiro gravado no banco confirma "Compute:Arena" 3x (numa delas com o prefixo cru "HN: Show HN: ..."), "Axiom" 2x e "usar Claude para criar jogos" 2x, e ainda inventa a data ("20 de setembro de 2026"). A coleta daquela execucao foi conferida e NAO tem duplicatas (0 pares com mesma URL ou titulo similar): a causa era o prompt, que pedia "agrupar as noticias por temas" e fazia o LLM reciclar itens para preencher os blocos. Correcao na origem: PROMPT_SISTEMA (src/boletim.py) reescrito com ESTRUTURA LINEAR (sem blocos tematicos), mencao unica por noticia, obrigacao de cobrir todas as noticias, proibicao de citar fonte/prefixo/URL e de citar data/dia/mes/ano/horario, concisao (~2.400 chars, 1-2 frases por noticia); a REGRA CRITICA anti-alucinacao foi mantida integral; em /api/chat entraram repeat_penalty 1.2 e repeat_last_n 1024 (temperature 0.6 mantida). Defesa em profundidade: src/coletor.py deduplica a saida (URL normalizada ou titulos com similaridade >= 0.85 via difflib; mantem a primeira ocorrencia, HN antes do Reddit) e informa quantas noticias descartou. Validacao (tarefa 13): compileall OK; coleta de 20 itens com "0 noticia(s) descartada(s) por duplicidade entre fontes" e "pares suspeitos restantes: 0"; roteiro gerado em memoria com 1440 chars e contagem de palavra-chave 0 ou 1 por noticia (nenhuma repetida, sem "HN:"/URL/data). Nada foi escrito em disco (nenhum audio novo) e o audio de evidencia audio/boletim_2026-09-17.wav nao foi tocado.
...
| 13 | 2026-09-17 | Evita repeticao de noticias no roteiro | OK | diagnostico pelo roteiro do id=2 (Compute:Arena 3x, Axiom 2x, jogo com Claude 2x; coleta sem duplicatas); PROMPT_SISTEMA com formato linear, mencao unica, sem fonte/URL/data e limite de 2.400 chars; repeat_penalty 1.2 / repeat_last_n 1024; dedupe por URL e similaridade de titulo em coletor.py; README 7/9, Canvas R9 e PDF atualizados |
